...
from gen_utils import make_folder

logger = logging.getLogger(__name__)


# home = '/media/gnodj/W-DATS/python_scripts/DeepLearning/Skylight-DL/yolov5_mod'
# source = '/media/gnodj/W-DATS/test_train'
# train_cfg = '/media/gnodj/W-DATS/python_scripts/DeepLearning/Skylight-DL/yolov5_mod/models/custom_yolov5s.yaml'
# batch=2
# imgsize=640
# epochs=10
# weights = source+'/trained_weights'
# log = source+'/train_logs'
...
def train(hyp, args_dict, device, world_size, global_rank, total_batch_size, tb_writer=None):
    batch_size = args_dict['batch-size']
    resume = args_dict['resume']
    data = args_dict['data']
    cfg = args_dict['cfg']
    #hyp = args_dict['hyp']
    weights = args_dict['weights']
    img_size = args_dict['img-size']
    logdir = args_dict['logdir']
    name = args_dict['name']
    local_rank = args_dict['local-rank']
    evolve = args_dict['evolve']
    notest = args_dict['no-test']
    nosave = args_dict['no-save']
    bucket = args_dict['bucket']
    epochs = args_dict['epochs']
    single_cls = args_dict['single-cls']
    cache_images = args_dict['cache-images']
    workers = args_dict['workers']
    rect = args_dict['rect']
    noautoanchor = args_dict['noautoanchor']
    image_weights = args_dict['image-weights']
    device = args_dict['device']
    adam = args_dict['adam']
    sync_bn = args_dict['sync-bn']
    ...
    epochs, batch_size, total_batch_size, weights, rank = \
        epochs, batch_size, total_batch_size, weights, global_rank
    ...
    nc, names = (1, ['item']) if single_cls else (int(data_dict['nc']), data_dict['names'])  # number classes, names
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, single_cls,
                                            hyp=hyp, augment=True, cache=cache_images, rect=rect,
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, total_batch_size, gs, args_dict,
                                       hyp=hyp, augment=False, cache=cache_images and not notest, rect=True,
                                       rank=-1, world_size=world_size, workers=workers)[0]  # testloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Multi-scale
            multi_scale = args_dict['multi-scale']
            if multi_scale:
        ...
        for f1, f2 in zip([wdir / 'last.pt', wdir / 'best.pt', results_file], [flast, fbest, fresults]):
            if os.path.exists(f1):
                ...
                if str(f2).endswith('.pt'):  # is *.pt
                    ...
                    os.system('gsutil cp %s gs://%s/weights' % (f2, bucket)) if bucket else None  # upload
        # Finish
    ...
    if resume:  # resume an interrupted run
        ...
        #log_dir = increment_dir(Path(logdir) / 'exp', name)  # runs/exp1
        log_dir = make_folder(logdir, 'run')
    ...
    if not evolve:
        ...
        train(hyp, args_dict, device, world_size, global_rank, total_batch_size, tb_writer)
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            print(hyp)
            results = train(hyp.copy(), args_dict, device, world_size, global_rank, total_batch_size, tb_writer)
...
def trainer(args_dict):
    ...
    if resume:  # resume an interrupted run
        ...
        assert os.path.isfile(ckpt), 'ERROR: --resume checkpoint does not exist'
        with open(log_dir+'/opt.yaml') as f:
            args_dict = argparse.Namespace(**yaml.load(f, Loader=yaml.FullLoader))  # replace
        cfg, weights, resume = '', ckpt, True
        logger.info('Resuming training from %s' % ckpt)
    ...
    if not evolve:
        ...
        #train(hyp, args_dict, device, tb_writer)
        train(hyp, args_dict, device, world_size, global_rank, total_batch_size, tb_writer)
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            #results = train(hyp.copy(), opt, device)
            results = train(hyp.copy(), args_dict, device, world_size, global_rank, total_batch_size, tb_writer=None)
